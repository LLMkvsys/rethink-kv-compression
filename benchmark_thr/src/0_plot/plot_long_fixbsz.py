...
prompt_length_list = [4096, 6144, 8192, 10240]
bsz_list = [1]
...
##### parse quant logs
log_folder = "./0_quant_long_logs/"
# quant_policy_list = [(0, "None"), (2, "KIVI"), (4, "KIVI"), (8, "KIVI"), (2, "GEAR"), (4, "GEAR"), (8, "GEAR")]
quant_policy_list = [(0, "None"), (4, "KIVI"), (4, "GEAR")]

quant_long_full_data = {
    "prefill": [],
    "decoding": [],
}
...
for phase in ['prefill', 'decoding']: 
    ...
    for bsz in bsz_list:
        ...
        phase_data.append((prompt_length_list, thr_dict))
    ...
    quant_long_full_data[phase] = phase_data
    assert len(phase_data) == len(bsz_list)
    phase_data.clear
    # print(tab)
...
##### parse sparse logs
log_folder = "./0_sparse_long_logs/"
quant_policy_list = [(0, "H2O"), (0, "StreamingLLM")]

sparse_long_full_data = {
    "prefill": [],
    "decoding": [],
}
...
for phase in ['prefill', 'decoding']: 
    ...
    sparse_long_full_data[phase] = phase_data
    assert len(phase_data) == len(bsz_list)
    phase_data.clear
    # print(tab)
...
##### plot
for phase in ['prefill', 'decoding']:
    # for idx, bsz in enumerate([1, 2, 4, 8, 16]):
    for idx, bsz in enumerate([1]):
        print(bsz)
        if bsz != 1: continue # FIXME:
        ...
        # # NOTE: plot sparse long, the min prompt len of sparse settingas start from 1024?
        # if prompt_length >=1024:
        # x_data, y_data = sparse_long_full_data[phase][idx-2]
        x_data, y_data = sparse_long_full_data[phase][idx]
        for k, v in y_data.items():
            quant_name, policy_name = k.split('_')
            # print(f"{quant_name} {policy_name}")
            label = policy_name
        ...
        if phase == "prefill":
            ...
            plt.xlabel('KV Length', fontsize=18)
        ...
        plt.grid()
        plt.savefig(f"{output_folder}/long_phase_{phase}_bsz_{bsz}{suffix}.pdf", bbox_inches='tight')
        print(f"{output_folder}/long_phase_{phase}_bsz_{bsz}{suffix}.pdf", flush=True)
        # plt.show()