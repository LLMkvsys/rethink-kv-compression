...
if __name__ == "__main__":
    ...
    if (quant_method == "None" and args.quant_bits not in [0]) or \
        (quant_method in ["HEAD", "KIVI", "GEAR"] and args.quant_bits not in [2, 4, 8]) or \
    ...
    max_prefill_token_num = args.batch_size * (args.prompt_length + 128)
    ...
    if args.sparse_policy is not None: 
        ...
        engine_config = PytorchEngineConfig(tp=args.tp, quant_bits=0, block_size=cal_block_size(64,0), cache_max_entry_count=0.8,
                                            # max_prefill_token_num=tokenizer.model_max_length,
                                            max_prefill_token_num=max_prefill_token_num,
                                            session_len=tokenizer.model_max_length,
                                                compression_config={
    ...
    # log_file = f"0_logs/decoding_model_{model_ident}_batch_size_{args.batch_size}_tp_{args.tp}_quant_bits_{args.quant_bits}_quant_policy_{quant_method}_prompt_length_{args.prompt_length}.log"
    log_file = f"0_quant_normal_logs/decoding_model_{model_ident}_batch_size_{args.batch_size}_tp_{args.tp}_quant_bits_{args.quant_bits}_quant_policy_{quant_method}_prompt_length_{args.prompt_length}.log"
