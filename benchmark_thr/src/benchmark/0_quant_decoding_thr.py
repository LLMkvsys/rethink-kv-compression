...
if __name__ == "__main__":
    ...
    for idx, json_obj in tqdm(enumerate(data), total=len(data)):
        ...
        if (idx + 1) % batch_size  == 0: 
            ...
            model_ident = model_path.replace('/', '-')
            with open(f"benchmark/logs/decoding_model_{model_ident}_batch_size_{args.batch_size}_tp_{args.tp}_quant_bits_{args.quant_bits}_quant_policy_{quant_method}_prompt_length_{args.prompt_length}.log", "w") as f: 
