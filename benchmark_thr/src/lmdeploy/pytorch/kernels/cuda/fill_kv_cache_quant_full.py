...
def _quant_int2(val1, val2, val3, val4):
    val_min_1 = tl.minimum(tl.min(val1, 1), tl.min(val2, 1))
    val_min_2 = tl.minimum(tl.min(val3, 1), tl.min(val4, 1))
    val_min = tl.minimum(val_min_1, val_min_2)
    
    val_max_1 = tl.maximum(tl.max(val1, 1), tl.max(val2, 1))
    val_max_2 = tl.maximum(tl.max(val3, 1), tl.max(val4, 1))
    val_max = tl.maximum(val_max_1, val_max_2)
    
    scales = (val_max - val_min) / 4
    zeros = -val_min / scales
    q_val1 = (val1 / scales[:, None] + zeros[:, None] + 0.5).to(tl.uint8)
    q_val2 = (val2 / scales[:, None] + zeros[:, None] + 0.5).to(tl.uint8)
    ...
    q_val = q_val1 + q_val2 * 4 + q_val3 * 4 * 4 + q_val4 * 4 * 4 * 4
    return q_val, scales, zeros
...
@triton.jit
def _fill_kv_cache_only_quant_kernel(
    KStates,
    VStates,
    KCaches,
    VCaches,
    ...
    for bidx in range(c_first_tokenloc, c_last_tokenloc):
        ...
        if quant_bits == 4:
            ...
            k2 = tl.load(ks_ptr + sidx * stride_kss +
                         ...
                         d_off[None, :] * stride_ksd + head_dim * stride_ksd,
                         mask=mask)
            k3 = tl.load(ks_ptr + sidx * stride_kss +
                         h_off[:, None] * stride_ksh +
                         d_off[None, :] * stride_ksd + 2 * head_dim * stride_ksd,
                         mask=mask)
            k4 = tl.load(ks_ptr + sidx * stride_kss +
                         h_off[:, None] * stride_ksh +
                         d_off[None, :] * stride_ksd + 3 * head_dim * stride_ksd,
                         mask=mask)
            
            q_k, k_scales, k_zeros = _quant_int2(k1, k2, k3, k4)
        ...
        if BLOCK_DV > 0:
            if quant_bits == 4:
                ...
                v2 = tl.load(vs_ptr + sidx * stride_vss +
                             h_off[:, None] * stride_vsh +
                             dv_off[None, :] * stride_vsd +
                             head_dim_v // 4 * stride_vsd,
                             mask=maskv)
                v3 = tl.load(vs_ptr + sidx * stride_vss +
                             h_off[:, None] * stride_vsh +
                             dv_off[None, :] * stride_vsd +
                             head_dim_v // 4 * 2 * stride_vsd,
                             mask=maskv)
                v4 = tl.load(vs_ptr + sidx * stride_vss +
                             h_off[:, None] * stride_vsh +
                             dv_off[None, :] * stride_vsd +
                             head_dim_v // 4 * 3 * stride_vsd,
                             mask=maskv)
                q_v, v_scales, v_zeros = _quant_int2(v1, v2, v3, v4)
...
@triton.jit
def _fill_kv_cache_only_full_kernel(
    KStates,
    VStates,
    KCaches,
    VCaches,
    ...
    block0_first_tokenloc = history_seqlen % BLOCK # it is applicable in decoding stage 

    state_token_offset = tl.maximum((block_id + (q_seqlen // BLOCK)) * BLOCK - block0_first_tokenloc,
                                    0)
    # kv_block_id = _div_up(history_seqlen + 1, BLOCK) - 1 + block_id
    # kv_block_id = min(kv_block_id, stride_boff - 1)
    # block_off = tl.load(BlockOffsets + batch_id * stride_boff + kv_block_id)
    block_off = tl.load(BlockOffsets + batch_id * stride_boff) # only use the first full precision block
    ...
    if block_id != 0:
        c_first_tokenloc *= 0
    c_last_tokenloc = tl.minimum(
        BLOCK, q_seqlen + block0_first_tokenloc - (block_id + q_seqlen // BLOCK) * BLOCK)
...
# @calculate_time(show=True, min_cost_ms=1, signature='fill_kv_cache')
def fill_kv_cache_quant_full(k_states: Tensor,
                  v_states: Tensor,
                  k_caches: Tensor,
                  v_caches: Tensor,
                  k_full_caches: Tensor, 
                  v_full_caches: Tensor,
                  q_start_loc: Tensor,
                  q_seq_length: Tensor,
    ...
    assert quant_bits > 0, 'this is designed for KV cache quantization only'
    
    if len(q_seq_length) > 0:
        if q_seq_length[0].item() == kv_seq_length[0].item():
            _fill_kv_cache_only_quant_kernel[grid](
                k_states,
                v_states,
                k_caches,
                v_caches,
        ...
        if False: # kv_seq_length[0].item() % block_size > 0:
            grid = [batch_size, 1]
            _fill_kv_cache_only_full_kernel[grid](
                k_states,
                v_states,
                k_full_caches,
                v_full_caches,
                ...
                stride_vsh=v_states.stride(-2),
                stride_vsd=v_states.stride(-1),
                stride_kcn=k_full_caches.stride(0),
                stride_kcb=k_full_caches.stride(1),
                stride_kch=k_full_caches.stride(2),
                stride_kcd=k_full_caches.stride(3),
                stride_vcn=v_full_caches.stride(0),
                stride_vcb=v_full_caches.stride(1),
                stride_vch=v_full_caches.stride(2),
                stride_vcd=v_full_caches.stride(3),
                stride_boff=full_block_offsets.stride(0),
                BLOCK=BLOCK,
                BLOCK_D=BLOCK_D,
                BLOCK_DV=BLOCK_DV,
                BLOCK_H=BLOCK_H,
