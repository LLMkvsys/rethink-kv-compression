...
from .pagedattention import _unpack_kv_int2, _unpack_kv_int2_transposed
...
def _fwd_split_kernel_quant(
    ...
    if quant_bits == 4:
        ...
        off_k = (cur_kv_head * stride_kh +
                 tl.arange(0, BLOCK_DMODEL // 4)[None, :] * stride_kd)
        off_v = (cur_kv_head * stride_vh +
                 tl.arange(0, BLOCK_DMODEL // 4)[None, :] * stride_vd)
    else:
        off_k = (cur_kv_head * stride_kh + offs_d[None, :] * stride_kd)
        off_v = (cur_kv_head * stride_vh + offs_d[None, :] * stride_vd)
...
def alibi_paged_attention_fwd(
    ...
    if quant_bits == 4:
        ...
        assert Lq == Lk * 4 and Lk == Lv
        assert Lk in {4, 8, 16, 32}
    else:
