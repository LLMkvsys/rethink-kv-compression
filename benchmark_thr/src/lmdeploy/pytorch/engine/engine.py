...
def _check_finish(scheduler: Scheduler, current_iter: int):
    ...
    # print("max_prefill_interval ", max_prefill_interval)
    # print("max_batches is ", max_batches)
    # print("num_batches is ", num_batches)
    # print("current_iter is ", current_iter)
    # print("min_iter is ", min_iter)
...
class Engine:
    ...
    def __init__(self,
        ...
        cache_config = CacheConfig(
            ...
            quant_bits=engine_config.quant_bits,
            quant_policy=engine_config.compression_config["quant_policy"],
        )
    ...
    def create_model_inputs(self, messages: SeqList, adapters: AdapterList,
        ...
        history_ignore_lengths = [msg.history_ignore_len for msg in messages]
        history_ignore_lengths = torch.tensor(history_ignore_lengths)
        ...
        # if is_decoding: 
        #     if token_ids[0] == 3892 or token_ids[0] == 13:
        ...
        block_full_offsets = self.scheduler.get_full_block_tables(messages)
        block_full_offsets = _tensorlize_block_offsets(block_full_offsets)
        ...
        return ModelInputs(
            ...
            history_lengths=history_lengths,
            history_ignore_lengths=history_ignore_lengths,
            block_offsets=block_offsets,
            block_full_offsets=block_full_offsets,
    ...
    async def _async_step_background(
        ...
        def __update_inputs(next_token_ids):
            ...
            # print("__update_inputs is ", next_token_ids)
            # if next_token_ids[0] == 3892 or next_token_ids[0] == 13: 
            #     import pdb; pdb.set_trace() 
        ...
        for idx in range(loop_count):
            ...
            # print("--------- async step background next_token_ids is ", next_token_ids, finish)
            # print('stopped is ', stopped)
            # print("_check_finish(self.scheduler, idx) is ", _check_finish(self.scheduler, idx))
    ...
    async def _async_loop(self):
        ...
        def __send_resp(out: InferOutput):
            ...
            # print("__send_resp is ", out.token_ids) # 3892
        ...
        async def __step(prefill: bool):
            ...
            while not finish:
                ...
                # if finish: 
                #     print("finish, out = await out_que.get() is ", finish, out)
                try:
