...
class CacheEngine:
    ...
    def __init__(
        ...
        if cache_config.quant_bits > 0:
            self.kv_cache_dtype = torch.uint8
        
        self.max_extra_full_blocks = cache_config.max_batches + 3
    ...
    def _get_key_block_shape_impl(cls,
        ...
        if quant_bits == 4:  # pack head_dim to uint8
            assert head_size % 2 == 0, \
                f'head_size: {head_size}, quant_bits: {quant_bits}'
            head_size = head_size // 2
        if quant_bits == 2: 
            # pack head_dim to uint8
    ...
    def _get_value_block_shape_impl(cls,
        ...
        if quant_bits == 2:  # pack head_dim to uint8
            assert head_size % 4 == 0, \
                f'head_size: {head_size}, quant_bits: {quant_bits}'
            head_size = head_size // 4
    ...
    def get_key_block_shape(self, local: bool = False) -> Tuple[int, int, int]:
        ...
        return self._get_key_block_shape_impl(
            ...
            quant_bits=self.cache_config.quant_bits,
            local=local,
        )
    ...
    def allocate_gpu_cache(self):
        ...
        for _ in range(self.num_layers):
            ...
            if self.cache_config.quant_bits in (2, 4, 8):
                if self.cache_config.quant_policy in ["HEAD", "GEAR"]: 
                    ...
                    gpu_cache.append(
                        ...
                        value_scales_zeros, self.cache_config.quant_bits))
                elif self.cache_config.quant_policy in ["KIVI"]: 
                    key_scales_zeros = torch.empty(
                        size=(self.num_gpu_blocks, *key_block_shape[:-1], 2),
                        dtype=self.model_config.dtype,
                        device='cuda',
                    ...
                    tmp_quant_bits = self.cache_config.quant_bits
                    self.cache_config.quant_bits = 0
                    full_key_block_shape = self.get_key_block_shape(local=True)
                    full_value_block_shape = self.get_value_block_shape(local=True)
                    self.cache_config.quant_bits = tmp_quant_bits
                    
                    key_full_blocks = torch.empty(
                        size=(self.max_extra_full_blocks, *full_key_block_shape),
                        dtype=torch.bfloat16,
                        device='cuda',
                    )
                    value_full_blocks = torch.empty(
                        size=(self.max_extra_full_blocks, *full_value_block_shape),
                        dtype=torch.bfloat16,
                        device='cuda',
                    )
                    
                    gpu_cache.append(
                        (key_blocks, value_blocks, key_scales_zeros, value_scales_zeros, 
                         key_full_blocks, value_full_blocks,
                        self.cache_config.quant_bits))
            else:
