...
class BaseModelAgent(AutoModelAgent):
    ...
    def _build_model(self, model_path: str, device: torch.device = 'cuda'):
        ...
        logger.info('add kv compression attributes.')
...
def build_model_agent(model_path: str,
                      ...
                      engine_config: [PytorchEngineConfig | TurbomindEngineConfig] = None):
    """create model agent."""
    model_config = ModelConfig.from_pretrained(
        model_path, trust_remote_code=trust_remote_code)
    model_config.custom_module_map = custom_module_map
    
    print("registering model compression policy ... ", engine_config.compression_config)
    model_config.compression_config = engine_config.compression_config
    if tp == 1:
