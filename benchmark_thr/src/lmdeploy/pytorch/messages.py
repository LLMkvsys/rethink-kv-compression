...
class HistoryTokenIds:
    ...
    def trim(self, start, end): 
        self._token_ids = self._token_ids[start:end]
        self._num_real = len(self._token_ids)
...
class SchedulerSequence:
    ...
    logical_full_blocks: LogicalTokenBlocks = field(
        default_factory=LogicalTokenBlocks)
    sender_id: int = -1
    req_id: int = -1
    adapter_name: str = None
    ...
    evict_blocks: np.ndarray = np.array([])
    ...
    def __post_init__(self):
        ...
        self._num_ignore_token_ids: int = 0
    ...
    @property
    def history_ignore_len(self) -> int:
        """get history length."""
        return self._num_ignore_token_ids
    ...
    def token_ids(self) -> np.ndarray:
        ...
        # if end - start < 10: 
        #     print('return token_ids == ', self.history_cache[start:end], start, end)
    ...
    def update_token_ids(self,
        ...
        # print("self.history_cache._token_ids[self.history_cache._num_real-1] == ", self.history_cache._token_ids[self.history_cache._num_real-5:self.history_cache._num_real])
        # print("update tokens == ", token_ids)
        # print("self.history_cache._num_real == ", self.history_cache._num_real)
        # import pdb; pdb.set_trace() 
