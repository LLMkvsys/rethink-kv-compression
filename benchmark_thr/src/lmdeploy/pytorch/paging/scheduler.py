...
class Scheduler:
    ...
    def _schedule_prefill(self):
        ...
        while len(waiting) > 0 and len(running) < max_batches:
            ...
            self.block_manager.allocate_full(seq)
    ...
    def _schedule_decoding(self, prealloc_size: int = 0):
        ...
        # 1. running
        for seq in running:
            # print("seq.token_ids == ", seq.token_ids)
            # import pdb; pdb.set_trace() 
            # token + n 
            if len(seq.evict_blocks) > 0: 
                self.block_manager.free_by_blocks(seq.evict_blocks)
                seq.evict_blocks = np.array([])
    ...
    def get_full_block_tables(self, seqs: Union[SeqList, AdapterList]):
        """get block table of the sequences."""
        return [self.block_manager.get_full_block_table(seq) for seq in seqs]