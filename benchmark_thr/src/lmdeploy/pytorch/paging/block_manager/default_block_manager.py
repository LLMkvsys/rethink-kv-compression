...
class DefaultBlockManager(BaseBlockManager):
    ...
    def allocate_msg_full(self, msg: SchedulerSequence):
        ...
        logical_blocks = msg.logical_full_blocks
        if len(logical_blocks) == 0:
            blocks = self.allocator.allocate_full(1, 'gpu')
            logical_blocks.append(blocks)
    ...
    def free(self, msg: SchedulerSequence):
        ...
        self.allocator.free_full(msg.logical_full_blocks.get_real_blocks())
        msg.logical_blocks.reset()
        # import pdb; pdb.set_trace()

    def free_by_blocks(self, blocks: np.ndarray): 
        self.allocator.free_by_blocks(blocks)
