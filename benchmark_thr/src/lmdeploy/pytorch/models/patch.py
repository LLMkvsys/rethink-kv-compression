...
def build_model_from_hf_config(model_config: PretrainedConfig,
    ...
    model = model_cls(model_config, ctx_mgr, dtype=dtype, device=device, extended_config=extended_config)
    # import pdb; pdb.set_trace()
    return model.eval()


def build_snap_kv_model(model_name_or_path): 
    import torch
    from ..cutils.sparsekv_monkeypatch import replace_llama, replace_mistral, replace_llama_forward_timing
    from transformers import LlamaForCausalLM
    eviction_method = "streamingllm"
    # sparsekv_args = parsed_args_dict["sparsekv_args"]
    max_capacity_prompts = 512 # sparsekv_args.max_capacity_prompts
    window_sizes = 8 # sparsekv_args.window_sizes
    kernel_sizes = 7 # sparsekv_args.kernel_sizes
    pooling = 'maxpool' # sparsekv_args.pooling

    replace_llama("streamllm")
    print(f"replace llama successfully")
    attn_impl = "flash_attention_2"
    print(f"current attn_implementation: {attn_impl}")
    ...
    if eviction_method in ["snapkv","pyramidkv","h2o"]:
        window_sizes = 8
    elif eviction_method in ["streamingllm"]:
...
def build_patched_model(config: ModelConfig, device: torch.device = None):
    ...
    dtype = config.dtype
    return build_model_from_hf_config(model_config, dtype=dtype, device=device, extended_config=config.compression_config)
