...
from utils.sparsekv_utils import init_pyramidkv,init_snapkv,init_H2O,init_StreamingLLM
import math
import time
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len:
            ...
            # assert past_key_value._seen_tokens == 0
    ...
    if self.layer_idx == 0: past_key_value._seen_tokens = self.kv_seq_len
...
def llama_flash_attn2_forward_H2O(
    ...
    **kwargs,
    ...
    # FIXME: to upgrade to adapt to transformers==4.43.1
    # cos, sin = self.rotary_emb(value_states, position_ids)
    cos, sin = self.rotary_emb(value_states, position_ids)
    query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)
...
def LlamaForCausalLM_forward_timing(
        self,
        ...
        # NOTE: XINYUZHOU, record the prefilling completion timestamp
        g_timer.forward_count += 1
        if g_timer.forward_count == 1:
            assert outputs.past_key_values is not None
            g_timer.prefill_times.append(time.time())
