...
class TurbomindEngineConfig:
    ...
    Args:
        ...
        quant_bits (int): default to 0. When k/v is quantized into 8 bit, set it to 4
    ...
    def __post_init__(self):
        ...
        assert self.quant_bits in (0, 4, 8), 'invalid quant_bits'
...
class PytorchEngineConfig:
    ...
    revision: str = None
    quant_bits: Literal[0, 2, 4, 8] = 4
    compression_config: dict = field(default_factory=lambda: {"compression_method": "None"}) # FIXME: 
    ...
    def __post_init__(self):
        ...
        assert self.quant_bits in (0, 2, 4, 8), 'invalid quant_bits'
        if "compression_method" in self.compression_config: 
            # import pdb; pdb.set_trace() 
            assert self.compression_config["compression_method"] in [ "None", "H2O", "StreamingLLM", "KIVI", "HEAD", "GEAR"], 'invalid compression_method'
