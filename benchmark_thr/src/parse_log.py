...
for phase in ['prefill', 'decoding']: 
    ...
    for bsz in [1, 2, 4, 8]: 
        for prompt_length in [256, 512, 1024, 2048, 3072]: # , 4096]: 
            time_infos = list() 
            Q_Policy = "KIVI"
            # Q_Policy = "HEAD"
            # for (quant, policy) in [(0, "None"), (8, "KIVI"), (4, "KIVI")]: 
            for (quant, policy) in [(0, "None"), (8, Q_Policy), (4, Q_Policy)]: 
                path = os.path.join("0_logs", f"{phase}_batch_size_{bsz}_tp_1_quant_bits_{quant}_quant_policy_{policy}_prompt_length_{prompt_length}.log")
