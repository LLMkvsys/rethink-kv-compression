...
def find_common_failures(folder_path, model_name, base_alg, other_algs, all_datasets, threshold):
    ...
    for dataset_name, files in datasets.items():
        ...
        if baseline_file:
            ...
            if len(failure_ids) > 0: 
                ...
                other_score_list = [other_data_list[0][failure_id]['score'] for failure_id in failure_ids]
                base_score_info[dataset_name] = np.mean(base_score_list)
                other_score_info[dataset_name] = np.mean(other_score_list)
    ...
    # import pdb; pdb.set_trace() 
    print(f"base score is {np.mean(list(base_score_info.values()))}, other score is {np.mean(list(other_score_info.values()))}")
    if False: 
        results = {}
...
for other_algs in [kivi_alg_idents]: 
    print("-" * 20)
