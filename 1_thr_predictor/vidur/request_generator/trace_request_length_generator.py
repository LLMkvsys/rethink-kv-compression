...
def recip_df(df):
    # return df 
    # df = df[['num_decode_tokens', 'num_prefill_tokens', 'num_total_tokens', 'pd_ratio']]
    df['num_decode_tokens'], df['num_prefill_tokens'] = df['num_prefill_tokens'], df['num_decode_tokens']
    # np.random.seed(42)
    # df['num_decode_tokens'] = np.random.randint(128, 2048, size=len(df))
    # df['num_prefill_tokens'] = np.random.randint(128, 2048, size=len(df))
    df['pd_ratio'] = df['num_prefill_tokens'] / df['num_decode_tokens']
    return df 
...
class TraceRequestLengthGenerator(BaseRequestLengthGenerator):
    ...
    def __init__(self, config: TraceRequestLengthGeneratorConfig):
        ...
        self.trace_df = recip_df(self.trace_df)
