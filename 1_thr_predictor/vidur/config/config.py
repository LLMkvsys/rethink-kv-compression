...
class VllmSchedulerConfig(BaseReplicaSchedulerConfig):
    ...
    sort_priority: str = field(
        default="arrival",
        metadata={"help": "sort priority."},
    )
    @staticmethod
    def get_type():
...
@dataclass
class SrtfSchedulerConfig(BaseReplicaSchedulerConfig):
    max_batched_tokens: int = field(
        default=None,
        metadata={"help": "Maximum batched tokens for srtf."},
    )
    max_tokens_in_batch: int = field(
        default=4096,
        metadata={"help": "Maximum tokens in batch for srtf."},
    )
    ...
    @staticmethod
    def get_type():
        return ReplicaSchedulerType.SRTF
...
class ClusterConfig:
    ...
    global_scheduler_sort_priority: str = field(
        default="arrival",
        metadata={"help": "Global scheduler priority config."},
    )
